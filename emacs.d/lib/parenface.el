...
(defvar paren-face-lisp-regexp "(\\|)")
(defvar paren-face-perl-regexp 
  "(\\|)\\|{\\|}\\|\\[\\|\\]\\|\\$\\|&\\|*\\|@\\|->\\|%")

(defmacro paren-face-add-support (regexp keywords)
  "Generate a lambda expression for use in a hook."
  `(lambda ()
    (let* ((match (assoc ,regexp ,keywords)))
      (unless (eq (cdr match) paren-face)
        (setq ,keywords (append (list (cons ,regexp paren-face))
                                ,keywords))))))
...
(add-hook 'scheme-mode-hook
          (paren-face-add-support paren-face-lisp-regexp
...
(add-hook 'perl-mode-hook
          (paren-face-add-support paren-face-perl-regexp
